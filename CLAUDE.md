...
**Auth**: phone-only, no email anywhere (migrated away in backend `V6`/`V7`; frontend has zero email references). Signup collects profile fields + password in one step, no OTP. Login is two-step: `POST /api/auth/login` → OTP challenge → `POST /api/auth/verify-otp` → real JWTs. This is the **original custom system**, unchanged — see "Known-reverted work" below. New signups also land in an unapproved state pending admin review (backend `V8`/`users.approved`, see [audio/CLAUDE.md](audio/CLAUDE.md)) — a signed-up-but-unapproved user still gets a working JWT immediately, but their *next* login attempt is rejected with a `403`/`"You have not been approved yet"` until an admin approves them from Manage Users; `login_screen.dart` catches that exact error string to show a dedicated "pending approval" dialog rather than a generic error toast.
